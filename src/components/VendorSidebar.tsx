...
const menuItems = [
  ...
  { icon: Truck, label: "My Vehicles", href: "/vendor/vehicles" },
  { icon: Users, label: "My Drivers", href: "/vendor/drivers" },
  { icon: Map, label: "Trips Assigned", href: "/vendor/trips" },
];
  ...
  return (
    <div className="w-64 min-h-screen bg-slate-950 text-slate-300 flex flex-col border-r border-slate-800 shadow-2xl">
      <div className="p-6 flex items-center gap-3">
        <div className="w-8 h-8 bg-amber-600 rounded-lg flex items-center justify-center font-bold text-white text-xl">V</div>
        <h1 className="text-xl font-bold text-white tracking-tight">Vendor Portal</h1>
      </div>
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-amber-600/20 flex items-center justify-center text-amber-500 font-bold border border-amber-500/20">
              {session?.user?.name ? session.user.name[0].toUpperCase() : "V"}
            </div>
            <div className="overflow-hidden">
              <p className="text-sm font-medium text-slate-200 truncate pr-2">{session?.user?.name || "Vendor"}</p>
              <p className="text-xs text-slate-500">Fleet Partner</p>
            </div>
          </div>
        </div>
        <button 
